...
uniao_disjunta_re = re.compile("^\s*(A|B)\s*\\\\\s*(A|B)\s*u\s*(A|B)\s*\\\\\s*(A|B)\s*$")
...
while __name__ == '__main__':
    ...
    if m:
        print(m.group(1), "=", "{" + ', '.join(str(v) for v in sorted(conj[m.group(1)])) + "}")
        continue
    ...
    if m:
        ...
        print(m.group(1), "u", m.group(2), "=", "{" + ', '.join(str(v) for v in r) + "}")
        continue

    m = intersecao_re.match(linha)
    if m:
        r = sorted(lab.intersecao(conj[m.group(1)], conj[m.group(2)]))
        print(m.group(1), "^", m.group(2), "=", "{" + ', '.join(str(v) for v in r) + "}")
        continue
